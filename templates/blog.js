...
const { STATUS_INTERFACE_CHOICES, STATUS_DISPLAY_CHOICES, TOOLBAR_BASIC } = require("./constants");
...
const model = build((builder) => {
  ...
  posts.string("slug", 90).notNullable().unique().interface("input", { trim: true, slug: true }).required();
  posts.text("excerpt").notNullable().interface("input-multiline", { trim: true }).required();
  posts.text("body").notNullable().interface("input-rich-text-html", { toolbar: TOOLBAR_BASIC }).required();
  posts
